...
``extensions/``
   ...
   ``extensions/gcc.bzl``, defines the tag classes and resolves user
   declarations into repository rule invocations.
...
``rules/``
   ...
   ``rules/qnx_sdp_gen.bzl`` handles the special case where a QNX SDP is built
   locally from the QNX Software Center installer.

``packages/``
   Stores package metadata and BUILD descriptors for supported toolchain
   archives. The most important file is ``packages/version_matrix.bzl``, which
   maps logical toolchain identifiers to URLs, checksums, build files, and any
...
``templates/``
   ...
   QNX because the execution environment, sysroot layout, and licensing model
   are different.
